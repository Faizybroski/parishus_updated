  ...
  useEffect(() => {
    ...
    if (user && profile && !authLoading && !profileLoading) {
      ...
      if (
        ...
        console.log(
          "✅ ProtectedRoute: User is not rejected, redirecting to home"
        );
        navigate("/", { replace: true });
        return;
      }
      ...
      if (currentPath === "/suspended-user" && !profile.is_suspended) {
        console.log(
          "✅ ProtectedRoute: User is not suspended, redirecting to home"
        );
        navigate("/", { replace: true });
        return;
      }
      ...
      if (profile.role === "admin" && currentPath === "/admin/login") {
        console.log(
          "🚫 ProtectedRoute: Admin already logged in, redirecting..."
        );
      ...
      if (profile.role === "admin") {
        ...
        console.log(
          "✅ ProtectedRoute: User has completed onboarding, staying on user routes"
        );
      }
    } else {
