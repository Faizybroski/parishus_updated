  ...
  try {
    ...
    if (!profile?.allow_crossed_paths_tracking) {
      return new Response(
        JSON.stringify({
          suggestions: [],
          message: "Crossed paths tracking disabled",
        }),
        {
          status: 200,
    ...
    if (restaurant_id) {
      ...
      if (sameRestaurantUsers) {
        suggestions = sameRestaurantUsers
          .filter((path: any) => {
            const otherProfile =
              path.user_a_id === profile.id ? path.profiles_b : path.profiles_a;
      ...
      if (nearbyUsers) {
        const filteredNearby = nearbyUsers
          .filter((path: any) => {
            ...
            if (
              dietary_theme &&
              otherProfile.dietary_preferences?.includes(dietary_theme)
            ) {
