      ...
      try {
        ...
        const { data: event, error: eventError } = await supabaseClient
          ...
          .insert({
            ...
            description:
              "A curated mystery dinner experience with fellow food enthusiasts in your area.",
            date_time: eventDate.toISOString(),
            location_name: getRandomLocation(
              group[0].location_city || "City Center",
            ),
  ...
  for (const user of users) {
    ...
    for (const otherUser of users) {
      ...
      const distance = calculateDistance(
        user.location_lat,
        user.location_lng,
        otherUser.location_lat,
        otherUser.location_lng,
      );
...
function getMostCommonDietaryPreference(group: any[]): string | null {
  ...
  return Object.keys(prefCounts).sort(
    (a, b) => prefCounts[b] - prefCounts[a],
  )[0];
}
...
function getMostCommonDiningStyle(group: any[]): string | null {
  ...
  return Object.keys(styleCounts).sort(
