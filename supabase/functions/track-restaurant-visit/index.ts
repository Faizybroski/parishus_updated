...
const handler = async (req: Request): Promise<Response> => {
  ...
  try {
    ...
    const {
      ...
      longitude,
      visited_at,
    }: RestaurantVisitRequest = await req.json();
    ...
    if (!profile?.allow_crossed_paths_tracking) {
      return new Response(
        JSON.stringify({ message: "Tracking disabled by user preference" }),
        {
          status: 200,
    ...
    for (const match of potentialMatches) {
      ...
      const userAId = user_id < otherUserId ? user_id : otherUserId;
      const userBId = user_id < otherUserId ? otherUserId : user_id;
